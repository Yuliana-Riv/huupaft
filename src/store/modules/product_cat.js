...
export default {
    ...
    actions:{
        setAddedProCat({commit}, payload){
            commit(SET_ADDED, payload);  
        },

        setHistoryOptionProCat({commit}, payload){
            commit(SET_HISTORY_OPTION, payload);  
        },
        async getAllInfoProCat({ commit }, payload) {
            commit(GETTING_INFO);
            commit(GETTING_MSG);
            //console.log(payload)
        ...
        async getAllInfoProCat2({ commit }, payload) {
        ...
        async getAllInfoProCat3({ commit }, payload) {
        ...
        async getInfoByIdProCat({ commit }, payload) {
            commit(GETTING_INFO);
            try {              
                let token = userService.getToken() 
                let headers =  {'Authorization': token} ;
        ...
        async searchProCat({ commit }, payload) {
            commit(GETTING_INFO);
            commit(GETTING_MSG);
        ...
        async deleteItemProCat({ commit }, payload) { 
        ...
        async editItemProCat({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
            try {               
                let token = userService.getToken() 
        ...
        async addItemProCat({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
        ...
        async addImagesProCat({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
        ...
        async deleteImageProCat({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
            try {               
                let token = userService.getToken() 
                let response = await api.deleteItem(payload, token);

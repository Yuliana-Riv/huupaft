...
import AddTg from "./addTg.vue";
import EditTg from "./editTg.vue";
export default {
  name: "Tag",
  components: {
    AddTg,
  ...
  computed: {
    data() {
        let data = this.$store.getters["tag/data"];
  ...
  methods: {
    ...
    changeView(option, item) {
      this.item = item;
      this.setAddedTg(item);
      this.setHistoryOptionTg(option);
    },
    ...
    eliminar: async function () {
      ...
      let result = await this.$store.dispatch("tag/deleteItemTg", {
        option: "tag",
        id: id,
      });

<template >
  <div id="EditCat">
    ...
    <form @submit.prevent="addValue(valor)">
      <div class="grup-form dflx">
        <div class="boxform">
          ...
          <input v-model="valor" type="text" id="valor" name="valor" required />
        </div>
      </div>
    ...
    <div class="ctn_tblv" v-if="added.values.length > 0">
      <div
        class="body_tbl_valores dflx"
        v-for="(item, key) in added.values"
        :key="key"
      >
        <p class="tbv-b1">{{ item.valor }}</p>
        <p class="tbv-b2" @click="(modalDelete = true), (itemDel = item.id)">
          &#10006;
        </p>
      </div>
    </div>

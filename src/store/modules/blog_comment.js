...
export default {
    ...
    actions:{
        setAddedBloCo({commit}, payload){
            commit(SET_ADDED, payload);  
        },

        setHistoryOptionBloCo({commit}, payload){
            commit(SET_HISTORY_OPTION, payload);  
        },
        async getAllInfoBloCo({ commit }, payload) {
            commit(GETTING_INFO);
            commit(GETTING_MSG);
            //console.log(payload)
        ...
        async getAllInfoBloCo2({ commit }, payload) {
        ...
        async getAllBloCo3({ commit }, payload) {
        ...
        async getInfoByIdBloCo({ commit }, payload) {
            commit(GETTING_INFO);
            try {              
                let token = userService.getToken() 
                let headers =  {'Authorization': token} ;
        ...
        async searchBloCo({ commit }, payload) {
            commit(GETTING_INFO);
            commit(GETTING_MSG);
        ...
        async deleteItemBloCo({ commit }, payload) { 
        ...
        async editItemBloCo({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
            try {               
                let token = userService.getToken() 
        ...
        async addItemBloCo({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS') 
            try {               
                let token = userService.getToken() 
        ...
        async addImagesBloCo({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
        ...
        async deleteImageBloCo({ commit }, payload) { 
            commit(SET_STATUS, 'SUCCESS')
            try {               
                let token = userService.getToken() 
                let response = await api.deleteItem(payload, token);

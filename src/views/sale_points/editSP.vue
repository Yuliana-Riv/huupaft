<template>
  <div id="EditBlo">
    <div class="form-content">
      <form @submit.prevent="submit(added.name, added.link, added.city, file1)">
        ...
        <div class="boxform">
          ...
          <div class="select-c">
            ...
            <p v-for="(d,i) in data" :key="i" class="pointer" @click="added.city = d.name">{{ d.name }}</p>
          </div>
        </div>
        <div class="grup-form dflx">
          <div class="boxform">
...
export default {
  ...
  computed: {
    ...
    added() {
      return this.$store.getters["sale_points/getAdded"];
    },
  },
  methods: {
    ...mapActions("sale_points", ["setAddedSP"]),
    ...mapActions("sale_points", ["setHistoryOptionSP"]),
    ...mapActions("sale_points", ["getInfoByIdSP"]),
    ...
    submit: async function (name, link, city, file1) {
      ...
      if (file1 != null) {
        ...
        let result = await this.$store.dispatch("sale_points/editItemSP", {
          option: "sale_points",
          item: data,
        });

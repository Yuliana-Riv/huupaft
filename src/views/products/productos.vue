<template>
  ...
  <p class="msg_text">CLASE - es un parametro requerido por  <strong> Skydropx</strong> para la creación del envío.</p>
            ...
            <div tbl_content>
                ...
                <div  v-if="data.length>0">
                    <div class="tbl_items dflx" v-for="(item, key) in filteredItems" :key="key">
                        <div class="tbl_item ">
                            <a :href="'/producto/'+item.url" target="_blank">{{item.name}}</a>
                        </div>
                        <div class="tbl_item ">
                        ...
                        <div class="tbl_item ">
                            <p class="three-dots"> <span class="asignado" v-if="item.skd_class">{{item.skd_class}}</span > <span class="alert" v-else>sin asignar</span></p>
                        </div>
                        <div class="tbl_item ">
                            <p>${{item.price}}</p>
                        </div>
...
export default {
       ...
       ...mapActions('product', ['getAllInfoPro']),
         ...
         ...mapActions('product', ['setAddedPro']),
           ...
           changeView(option, item){
             this.item=item
             this.setAddedPro(item)
             this.setHistoryOptionPro(option)
         },
 ...
 includesItem(search, name) {
        ...
        eliminar: async function(){
            ...
            let result = await this.$store.dispatch("product/deleteItemPro", {option: 'product', id:id});
